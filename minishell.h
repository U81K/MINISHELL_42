...
/*LEXER_2*/
t_info				*handle_output_redirection(t_info *info, char *line,
						int *i);
t_info				*handle_inpute_redirection(t_info *info, char *line,
						int *i);
...
void				joind_and_free_next(t_info *node, t_state state,
						t_info *to_delete);
...
t_info				*creat_node(t_info *head, char *content, t_type type,
						t_state state);
...
void				redirect_fd_to_pipe_and_close(int num_of_cmd, int (*fd)[2],
						int index);
...
t_env				*fork_multiple_command(t_cmd *cmd, t_env *env, t_info *info,
						int num);
...
/*exucution_2*/
void				to_write_error(char *cmd, char *msg, int len,
						int exit_status);
